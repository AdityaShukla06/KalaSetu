...
Background removal defaults to `BACKGROUND_REMOVAL_PROVIDER=none`, which means the studio's remove-background button reports it as unavailable and the rest of the pipeline is completely unaffected. Set it to a comma-separated list to turn it on, tried in order until one succeeds, same pattern as the craft classifier chain: `remove-bg` (a key from remove.bg, best quality, quota-limited) and/or `self-hosted` (your own instance of [`bg-removal-service`](bg-removal-service), no quota, see its README for why `u2netp` specifically was picked after measuring, not assuming, three real models' memory and speed). `remove-bg,self-hosted` runs remove.bg first and only falls through to the free self-hosted tier once remove.bg fails or its quota is spent. If every configured tier times out, hits its rate limit, or errors, `enhanceProductImage` catches it, logs the reason server side, and returns the artisan's photo through the ordinary `sharp` pipeline unchanged, with a short notice code the studio turns into a plain sentence. A slow or failed background removal call never blocks the listing and never loses the photo.
