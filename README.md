...
A bilingual (English / Hindi) mobile-first PWA that helps Indian artisans photograph, describe, price, and publish their products, using their voice instead of a keyboard.
...
src/                 the PWA
  ...
  context/           Auth, Language (with translations.ts), AddProductDraft
...
supabase/schema.sql  tables, index, counter function, RLS, storage bucket
```
...
| POST | `/api/voice/transcribe` | raw audio bytes, `?category=` | `{ transcript, descriptionEn, descriptionHi, detectedLanguage }` |
...
- Test UI changes in both English and Hindi. Hindi strings are longer and break layouts first.
- New user-facing copy goes into both `en` and `hi` in `src/context/translations.ts`.
