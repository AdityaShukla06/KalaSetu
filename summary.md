...
- **Profile.** Display name, shop name, region (used for buyer-side filtering), an optional WhatsApp number (turns on the WhatsApp button buyers see on that artisan's listings), and language, with a save confirmation.
...
- Eight schema migrations so far: multilingual product fields, the three-role model, buyer marketplace fields (region, material), the admin console (deactivation, review status, audit log), the Craft Heritage Passport (passport id, technique/time-taken/GI-tag/care-instructions, product story), the pricing overcharge auto-flag (`products.auto_flag_reason`), view tracking (the `product_views` table), and inquiry details plus a WhatsApp number (`inquiries.quantity/contact_preference/contact_value/read_at/responded_at/notified_at`, `users.whatsapp_number`). **Migrations 006, 007, and 008 have not been run yet** and need to be applied in the Supabase SQL Editor; 007 and 008 both fail soft (showing zero views, or a missing WhatsApp number, rather than erroring) everywhere except the inquiries feature itself, which genuinely needs 008 to work at all, the same way the passport and pricing features needed their own migrations.
- The ONDC catalog export needs no new schema at all; it's computed entirely from data already in `products`.

## Testing

- 285 automated tests (25 of them for the pricing overcharge flag, view tracking, and the inquiry flow, currently skipping until migrations 006, 007, and 008 are applied; 19 more for the ONDC catalog mapping and the WhatsApp link builder, which need no database and always run) cover login, the full artisan publish flow, role and permission boundaries, the marketplace search and filters, the entire admin console (access control, deactivation, moderation, the audit trail), the heritage passport (id format, public visibility rules, story stability), the pricing overcharge flag (capping, neutral wording, pricing freedom preserved), view tracking (recording, per-artisan isolation, honest zeroed summaries), the ONDC catalog mapping (correct field mapping, gaps always reported, cost and moderation data never leaking into an export), and the inquiry flow (contact-preference validation, read/responded tracking, per-artisan isolation, the email never losing the record).
...
- **Migrations 006, 007, and 008 have not been run against the live database yet.** `supabase/migrations/006-pricing-overcharge-flag.sql` adds the `auto_flag_reason` column (fails safe, does not block product creation), `supabase/migrations/007-product-views.sql` adds the `product_views` table (also fails safe, shows zero views), and `supabase/migrations/008-inquiry-details-and-whatsapp.sql` adds the inquiry detail columns and `users.whatsapp_number` (the WhatsApp button and profile field fail safe too, but the inquiry feature itself needs this one to actually work, the same way the passport and pricing features needed theirs). The twenty-five tests covering these three features skip themselves rather than fail in the meantime.
