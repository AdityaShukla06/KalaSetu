...
| POST | `/api/images/classify` | `{ imageUrl }` | `{ suggestion }`, artisan only, `suggestion` is `null` when every provider is unconfigured or fails |
...
| GET | `/api/inquiries/mine` | | `Inquiry[]` (each with a `messages` thread), buyer only, sent by the caller |
| GET | `/api/inquiries/received` | | `Inquiry[]` (each with a `messages` thread), artisan only, about the caller's products, marks them read as a side effect |
| PATCH | `/api/inquiries/:id` | `{ status: "closed" }` | `{ success }`, either party to the inquiry |
| POST | `/api/inquiries/:id/messages` | `{ body }` | `{ messageId, createdAt, emailDelivered }`, either party to the inquiry, emails the other side (best effort) |
...
**It's a real back-and-forth thread, not one message and one reply.** Every message either side sends, from the first one onward, lives as its own row in `inquiry_messages` (`inquiry_id`, `sender_role`, `body`, `created_at`), so a buyer and an artisan can go back and forth as many times as they need instead of the conversation stopping after a single reply. `GET /api/inquiries/mine` and `GET /api/inquiries/received` both return each inquiry's full `messages` array in order, and `POST /api/inquiries/:id/messages` is the one endpoint either party uses to add to it. Read state is tracked per side (`artisan_last_read_at`, `buyer_last_read_at` on `inquiries`), so each party's own "New" badge reflects only whether the other side has sent something since they last looked, independent of the other party's read state.

**Email never risks the record.** Creating an inquiry, or sending a follow-up message, inserts the row first; only after that succeeds does it attempt to email the other party via the existing Resend setup (`sendInquiryMessageEmail` in [`server/lib/mailer.ts`](server/lib/mailer.ts), same never-throws contract as the OTP mailer). The email includes the product name, its photo, the message body, and a deep link back into the recipient's own inbox (`PUBLIC_APP_URL/inquiries` for an artisan, `PUBLIC_APP_URL/marketplace/profile` for a buyer). If Resend is unreachable, misconfigured, or rejects the request, the message is already saved and visible in the thread either way.

**The artisan inbox** at `/inquiries`, linked from My Shop, lists every inquiry received with its full thread and a composer to reply or send another message at any time, plus a "Reply on WhatsApp" shortcut using the buyer's own contact value when they gave one. The buyer's own inquiry list, in their profile, works the same way. Either side can close an inquiry (`PATCH /api/inquiries/:id`); a closed inquiry keeps its full history visible but stops accepting new messages.

**Either side can translate a message they cannot read.** A buyer writing Odia and an artisan reading Hindi could previously see each other's messages but not understand them, which made the thread decorative. Every message from the other party now carries a "Translate to <your language>" button, and once pressed the bubble shows the translation with a line reading "Translated to <your language> from <their language>" plus a toggle back to the original. Both sides use the same [`InquiryThread`](src/components/InquiryThread.tsx) component, so this is one implementation rather than two.

**The sender's language is recorded, but treated as a hint rather than proof.** `inquiry_messages.body_language` (migration 015) stores the sender's own interface language at the moment they pressed send, which costs no extra model call and is right most of the time. It is not trusted as the answer, because it is wrong in the case that matters most: someone with an English interface typing romanised Hindi. So `POST /api/translate` takes either a known `from` (the deterministic path, used by the add-product description mirror) or a `hint` (the guess, used here), and in the hint case the model decides the source language and reports it back. That reported language is what the "from" in the label names, so the label can never claim a source the translation did not actually come from.

**Romanised text is a first class case, not an afterthought.** "kitne din lagenge" is Hindi, "koto din lagbe" is Bengali, and "entha samayam padutundi" is Telugu, but all three are Latin letters and a naive detector calls them English. Both the Groq and Gemini detection prompts name this case with examples and tell the model to judge by the words rather than the script. Verified live against Groq: all three are detected correctly, including the Telugu one when deliberately given a misleading `hint` of English. On the client, [`isWrittenInOwnScript`](shared/languages.ts) (a Unicode block test per script, covered by [`shared/languages.test.ts`](shared/languages.test.ts)) is why the Translate button still appears on a romanised message whose recorded language already matches the reader's: matching languages alone are not enough to conclude the reader can read it.

**Nothing is translated until asked.** There is no automatic translation on load, so an inbox of thirty messages costs zero model calls until someone presses a button; translations are held in component state and are not persisted. If a translation fails the original stays on screen with an inline error, and a message that turns out to already be in the reader's language says so instead of showing a pointless "original/translation" toggle over identical text.

**RLS** covers the new `inquiry_messages` table with its own `inquiry_messages_select_buyer`/`inquiry_messages_select_artisan`/`inquiry_messages_select_admin` policies, scoped through the parent `inquiries` row exactly like the existing inquiry policies.
...
**The 30-day chart is readable on a phone, not just decorative.** The daily-views line used to be a bare sparkline: no y axis, no gridlines, no value labels, and a fixed intrinsic width that scrolled sideways inside its card, which on a 360px screen could push the one day that mattered off the visible area entirely. It now measures its container with a `ResizeObserver` and draws to exactly that width, so all 30 days are always on screen with no horizontal scroll. It has a real y axis (integer-only ticks chosen from a fixed step ladder, so a shop with 4 views never gets a "2.5 views" gridline), hairline gridlines, a labelled x axis, both axis titles, and the busiest day's value printed directly on its peak. The left margin is computed from the widest y tick, so a shop with four-digit view counts does not clip its own axis. Date labels are placed by measured pixel gap rather than a fixed count: candidate positions closer than 70px are dropped, which is what stops "16 Aug" and "17 Aug" colliding on a short series.

**Reading a specific day.** Dragging or hovering anywhere on the plot snaps a crosshair to the nearest day and updates a readout line above the chart (the count, the word Views, and the full date). Left and right arrow keys do the same when the chart has keyboard focus, and the whole thing carries a translated `aria-label` summarising the total and the busiest day. The readout sits above the chart rather than floating over the point, because a bubble under the reader's own fingertip is unreadable on a phone and clips at the card edge near either end of the series. Dates are formatted with `Intl.DateTimeFormat` in the artisan's own language, falling back to the system locale if a language tag is not one the runtime knows.
...
**Category suggestion.** As soon as `/api/images/enhance` returns, the enhanced photo is sent to `/api/images/classify` in the background, without the artisan waiting on it; by the time they reach the category screen a few taps later, the matching tile is usually already picked, still one tap from being changed. Three vision providers are tried in order behind [`server/image-ai/classification`](server/image-ai/classification): Groq's Qwen vision models first, through the same key pool used for voice, then Gemini (constrained by a JSON schema to exactly the app's six category ids, so it cannot invent one), then a small model purpose-trained on these six craft categories, hosted separately. Groq goes first because it measured consistently fast (a couple of seconds); Gemini's schema-constrained vision calls measured 5-25 seconds in testing, which is too slow to sit first inside the [60-second serverless function budget](vercel.json) once every tier is counted, so it sits second instead. Each tier is only reached if the one before it is unconfigured or fails, and each has its own bounded timeout so one slow tier can't burn the whole request's budget, leaving nothing for the tiers after it; a quota outage on one provider never removes the feature, and with none configured at all, the category screen behaves exactly as before this existed. A prediction preselects its tile at any confidence level, since an occasional wrong guess costs one tap to correct, while showing nothing at all reads as the feature not working; a low-confidence guess is preselected the same way but carries a distinct "please check this" note instead of the usual one. Any tap the artisan makes themselves is never overridden by a suggestion that resolves later.
...
Background removal defaults to `BACKGROUND_REMOVAL_PROVIDER=none`, which means the studio's remove-background button reports it as unavailable and the rest of the pipeline is completely unaffected. Set it to `self-hosted` with `SELF_HOSTED_BG_REMOVAL_URL` pointing at your own instance of [`bg-removal-service`](bg-removal-service) to turn it on; no account, key, or quota anywhere, see its README for why `u2netp` specifically was picked after measuring, not assuming, three real models' memory and speed on Render's free tier. If it times out or errors, `enhanceProductImage` catches it, logs the reason server side, and returns the artisan's photo through the ordinary `sharp` pipeline unchanged, with a short notice code the studio turns into a plain sentence. A slow or failed background removal call never blocks the listing and never loses the photo.
...
**An optional second pricing engine, not live by default.** [`pricing-service/`](pricing-service/) is a standalone Python service implementing a different design: real crafting hours times a regional skilled wage for the cost floor, and a machine-learned market estimate (image and description embeddings retrieved against a catalogue of real priced listings) blended in by confidence, rather than a fixed labour multiplier of material cost. `server/routes/pricing.ts` tries it first when `PRICING_SERVICE_URL` is set, and falls back to the engine above on any failure, so pricing can never go down because of it. Leave `PRICING_SERVICE_URL` unset (the default) and nothing changes; see `pricing-service/README.md` for what it does and does not have yet, most importantly that its market catalogue is still a 16-row synthetic fixture, not real listings.
