...
**Category suggestion.** As soon as `/api/images/enhance` returns, the enhanced photo is sent to `/api/images/classify` in the background, without the artisan waiting on it; by the time they reach the category screen a few taps later, the matching tile is usually already picked, still one tap from being changed. Three vision providers are tried in order behind [`server/image-ai/classification`](server/image-ai/classification): Gemini first (constrained by a JSON schema to exactly the app's six category ids, so it cannot invent one), then Groq's Qwen vision models through the same key pool used for voice, then a small model purpose-trained on these six craft categories, hosted separately. Each tier is only reached if the one before it is unconfigured or fails, so a quota outage on one provider never removes the feature; with none configured at all, the category screen behaves exactly as before this existed. A prediction only preselects a tile at medium or high confidence; a low-confidence guess is shown as a badge but left unselected, and any tap the artisan makes themselves is never overridden by a suggestion that resolves later.
