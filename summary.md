...
- **Category suggested automatically from the photo.** By the time the artisan reaches the category picker, the right tile is usually already selected, guessed from the photo itself, always changeable with one tap. Three vision providers are tried in order (Gemini, then Groq, then a small model trained specifically on these six craft categories) so one provider running out of quota never removes the feature; a low-confidence guess is shown but never preselected, so a bad guess can't quietly mislabel a listing.
