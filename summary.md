...
- **Migrations 006 through 009 have not been run against the live database yet.** `supabase/migrations/006-pricing-overcharge-flag.sql` adds the `auto_flag_reason` column (fails safe, does not block product creation), `supabase/migrations/007-product-views.sql` adds the `product_views` table (also fails safe, shows zero views), `supabase/migrations/008-inquiry-details-and-whatsapp.sql` adds the inquiry detail columns and `users.whatsapp_number` (the WhatsApp button and profile field fail safe too, but the inquiry feature itself needs this one to actually work, the same way the passport and pricing features needed theirs), and `supabase/migrations/009-shipping-estimate.sql` adds `products.weight_kg` and `users.pincode` (fails safe, the shipping estimate just doesn't show until it's run). The thirty-one tests covering these four features skip themselves rather than fail in the meantime.
